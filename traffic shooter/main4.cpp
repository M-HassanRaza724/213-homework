...
void generate_e_Bullet(int enemy_idx){
    if(Ex[enemy_idx] == Px+3){
        e_bulletX[e_bulletCount[enemy_idx]][enemy_idx] = Ex[enemy_idx];
        e_bulletY[e_bulletCount[enemy_idx]][enemy_idx] = Ey[enemy_idx]+8;
...
void move_e_Bullet(int enemy_idx){
    for (int i = 0; i < e_bulletCount[enemy_idx]; i++)
    {
        if(is_e_BulletActive[i]){
            ...
            if(next != ' '){
                ...
                eraseBullet(e_bulletX[i][enemy_idx],e_bulletY[i][enemy_idx]);
                make_e_BulletInactive(i,enemy_idx);
            }else{
                eraseBullet(e_bulletX[i][enemy_idx],e_bulletY[i][enemy_idx]);
                e_bulletY[i][enemy_idx]++;
                printBullet(e_bulletX[i][enemy_idx],e_bulletY[i][enemy_idx]);
            }
    ...
    for (int j = 0; j < 3; j++)
    {
        for (int i = 0; i < e_bulletCount[j]; i++){
            if(is_e_BulletActive[i][j]){
                if((e_bulletX[i][j]>Px && e_bulletX[i][j]<Px+13) && e_bulletY[i][j] == Py){
                    gotoxy(0,41); cout<<"bullet collision detected by enemy "<<j<<endl; //debug Helper
                    return true;
                }
            }
        }
